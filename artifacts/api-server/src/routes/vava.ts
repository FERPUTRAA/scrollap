...
// ─── P2P Call Sessions Cache ──────────────────────────────────────────────────
// Stores Agora sessions captured live from VAVA WebSocket relay.
// These are P2P video call channels between two VAVA users — joinable as audience.
...
const CALL_SESSION_TTL_MS = 5 * 60 * 1000; // 5 minutes
...
// GET /api/vava/call-sessions — return all P2P call sessions captured from WS relay
vavaRouter.get("/vava/call-sessions", (_req: Request, res: Response) => {
  ...
  const sessions = Array.from(callSessionsCache.values())
    ...
    .map((s) => ({
      channel: s.channel,
      token: s.serverToken ?? s.vavaToken,   // cert-signed preferred
            ...
            if (opcode === 1 || opcode === 2) {
              ...
              if (text.includes("connected to server")) {
                ...
                try {
                  ...
                  if (creds) {
                    // Generate a server-side token (cert-signed, uid=0 anonymous viewer).
                    const serverToken = generateToken(AGORA_APP_ID, AGORA_APP_CERTIFICATE, creds.channel, 0);

                    // Store in global P2P call sessions cache (accessible via /api/vava/call-sessions)
                    pruneCallSessions();
