            ...
            if (opcode === 1 || opcode === 2) {
              ...
              if (text.includes("connected to server")) {
                ...
                try {
                  ...
                  if (creds) {
                    // Generate a server-side token as additional fallback.
                    // Use uid=0 (anonymous viewer — Agora assigns random UID).
                    const serverToken = generateToken(AGORA_APP_ID, AGORA_APP_CERTIFICATE, creds.channel, 0);
                    // IMPORTANT: send VAVA's own token as primary — it was generated by
                    // VAVA's backend for this exact channel and is the most likely to work.
                    // serverToken is tried by the frontend as a secondary fallback.
                    send("agora_session", {
                      appId: AGORA_APP_ID,
                      channel: creds.channel,
                      token: creds.token,      // VAVA original token (primary)
